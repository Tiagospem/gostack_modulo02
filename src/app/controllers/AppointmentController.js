...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    await Notification.create({
      content: `Novo agendamento de ${userRequest.name} para o ${formattedDate}`,
      user: provider_id,
    });
  ...
  async delete(req, res) {
    ...
    // verifica esta dentro do horario limite
    if (isBefore(dateWithsub, new Date())) {
      return res.status(401).json({
        error: 'you can only cancel appointment 2 hoursin advance',
      });
    }
